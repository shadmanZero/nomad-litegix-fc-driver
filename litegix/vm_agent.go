...
func (vm *firecrackerVMManager) addVMAgentToRootfs(mountDir string) error {
	...
	// Create agent script
	agentPath := mountDir + "/usr/local/bin/vm-agent"
	if err := os.MkdirAll(mountDir+"/usr/local/bin", 0755); err != nil {
...
# Create a simple command server
...
# Start agent in background
{
    while true; do
        if command -v nc >/dev/null 2>&1; then
            ...
            echo "VM-AGENT-READY" | nc -l -p 1024 &
            sleep 1
        else
            # Fallback: use socat or simple approach
            sleep 5
        fi
    done
} &

echo "VM Agent ready on port 1024"
`

	if err := os.WriteFile(agentPath, []byte(agentContent), 0755); err != nil {
		return fmt.Errorf("failed to write agent script: %w", err)
	}

	// Add agent to startup script
	startupScript := mountDir + "/etc/init.d/vm-agent"
	if err := os.MkdirAll(mountDir+"/etc/init.d", 0755); err == nil {
		startupContent := `#!/bin/sh
# VM Agent startup script
case "$1" in
start)
    echo "Starting VM Agent..."
...
`
		os.WriteFile(startupScript, []byte(startupContent), 0755)
